...
struct GameView: View {
    ...
    /// Multi-select filters. Empty = show all. Non-empty = OR (screenshot OR blurry OR duplicate).
    @State private var activeFilters: Set<PhotoFilter> = []
    @State private var sortOrder: SortOrder = .newest
    @State private var currentIndex: Int = 0
    @State private var showingFullScreen: PhotoItem?
    /// Photos user marked for delete; batch-deleted on "Delete X from library" (one OS confirmation).
    ...
    /// Title bar: app icon + "Trash Pic Up", white background, own bar
    ...
    /// Toolbar row below title: Stats, Filter (iOS), loading, count — moved down
    ...
    /// Add photo to pending-deletes "folder"; advance. No OS call yet.
    ...
    /// Called from TinderCardView swipe-left or trash button: add to pending, advance.
    private func deleteCurrent(_ photo: PhotoItem) {
        addToPendingAndAdvance(photo)
    }

    /// Batch-delete all pending (one OS confirmation).
...
struct TinderCardView: View {
    ...
    /// Photo ID we're loading for; used to ignore stale async completions.
    ...
    var body: some View {
        NavigationView {
            List {
                ...
                if photo.analysis?.isScreenshot == true || photo.analysis?.isBlurry == true {
                    LabeledContent("Type") {
                        HStack(spacing: 4) {
                            if photo.analysis?.isScreenshot == true {
                                Text("Screenshot").foregroundStyle(TrashPicUpTheme.accentMagenta)
                            ...
                            if photo.analysis?.isScreenshot == true, photo.analysis?.isBlurry == true {
                                Text(",").foregroundStyle(TrashPicUpTheme.textSecondary)
                            }
                            if photo.analysis?.isBlurry == true {
                                Text("Blurry").foregroundStyle(TrashPicUpTheme.accentOrange)
                            }
...
// Full-screen photo: sheet on both platforms (native swipe-down to dismiss, like Stats)
