...
#else
private struct ConfettiEmitterRepresentable: NSViewRepresentable {
    let duration: TimeInterval
    let onComplete: (() -> Void)?

    func makeNSView(context: Context) -> NSView {
        let view = ConfettiEmitterNSView()
        view.duration = duration
        view.onComplete = onComplete
        return view
    }
...
private class ConfettiEmitterNSView: NSView {
    var duration: TimeInterval = 2.5
    var onComplete: (() -> Void)?
    ...
    private func makeCell(color: (r: CGFloat, g: CGFloat, b: CGFloat), shape: Shape) -> CAEmitterCell {
        let cell = CAEmitterCell()
        let nsColor = NSColor(red: color.r, green: color.g, blue: color.b, alpha: 1)
        cell.contents = createConfettiImage(color: nsColor, shape: shape)?.cgImage(forProposedRect: nil, context: nil, hints: nil)
        cell.birthRate = 12
        cell.lifetime = 12
        cell.velocity = 250
        cell.velocityRange = 100
    ...
    private func createConfettiImage(color: NSColor, shape: Shape) -> NSImage? {
        let rect: CGRect = shape == .rectangle ? CGRect(x: 0, y: 0, width: 14, height: 10) : CGRect(x: 0, y: 0, width: 8, height: 8)
        let image = NSImage(size: rect.size)
        image.lockFocus()
        color.setFill()
        switch shape {
        case .rectangle: NSBezierPath(rect: rect).fill()
        case .circle: NSBezierPath(ovalIn: rect).fill()
        }
        image.unlockFocus()
        return image
    }
