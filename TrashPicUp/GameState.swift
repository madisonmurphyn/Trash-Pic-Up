...
class GameState: ObservableObject {
    ...
    /// Photo IDs user chose to keep; excluded from rotation (persisted across app restarts).
    ...
    /// Target space in bytes (used when goalByPhotos is false). Default 1 GB.
    ...
    /// Call when displaying goals to roll period forward if we're in a new period.
    ...
    /// Progress in current period: photos deleted this period.
    var goalProgressPhotos: Int {
        max(0, photosDeleted - photosDeletedAtPeriodStart)
    }
    
    /// Progress in current period: space freed this period (bytes).
    var goalProgressBytes: Int64 {
        max(0, spaceFreed - spaceFreedAtPeriodStart)
    }
    
    /// Target for current goal (photos or bytes depending on goalByPhotos).
    var goalTargetDisplay: String {
        if goalByPhotos {
            return "\(goalTargetPhotos) photos"
        } else {
            let formatter = ByteCountFormatter()
            formatter.allowedUnits = [.useGB, .useMB]
            formatter.countStyle = .file
            return formatter.string(fromByteCount: goalTargetBytes)
        }
    }
    
    /// Progress fraction (0...1 or more if over goal).
    ...
    /// Human-readable progress line for current period, e.g. "45 / 100 photos this month".
    ...
    /// Use when file size was captured before delete (assets may not report size after deletion).
